...
struct Amount {
    ...
    var btcFormat: NumberFormatter {
        ...
        format.generatesDecimalNumbers = true
        format.negativeFormat = format.positiveFormat.replacingCharacters(in: format.positiveFormat.range(of: "#")!, with: "-#")
        format.currencyCode = "DGB"

        switch maxDigits {
        case 2: // dgBits
            format.currencySymbol = "m\(Symbols.bits)\(Symbols.narrowSpace)"
            format.maximum = (C.maxMoney/C.satoshis)*100000 as NSNumber
        case 5: // mDGB
            format.currencySymbol = "m\(Symbols.bits)\(Symbols.narrowSpace)"
            format.maximum = (C.maxMoney/C.satoshis)*1000 as NSNumber
        case 8: // DGB
            format.currencySymbol = "\(Symbols.btc)\(Symbols.narrowSpace)"
            format.maximum = C.maxMoney/C.satoshis as NSNumber
        default:
            format.currencySymbol = "\(Symbols.bits)\(Symbols.narrowSpace)"
        }
